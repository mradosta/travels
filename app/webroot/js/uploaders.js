var uploaders = {
    ...
    uploaderObjects 		: new Array(),

	empty					: function() {

		if (uploaders.uploaderObjects.length > 0) {
			return false;
		} else {
			return true;
		}
	...
	bind					: function() {
		...
		$('.file-uploader').each(
			...
			function (index, element) {
				...
				if ($(element).attr('extensions') != undefined) {
					extensions = $(element).attr('extensions').split(',');
				...
				var resolution = '';
				if ($(element).attr('resolution') != undefined) {
					resolution = '/' + $(element).attr('resolution');
				}

				uploaders.uploaderObjects[index] = new qq.FileUploader({
					...
					onComplete: function(id, fileName, response) {
						uploaders.uploaderObjects[index].response = response;
					},

					// path to server-side upload script
					action: $.path($.parseJSON(info).current_controller + '/do_upload' + resolution)

				});

				uploaders.uploaderObjects[index].element = element;
    ...
    upload					: function() {
        for (var i = 0; i < uploaders.uploaderObjects.length; i++) {
            uploaders.uploaderObjects[i].uploadList();
        }
	...
	isUploading				: function() {
		...
		for (var i = 0; i < uploaders.uploaderObjects.length; i++) {
			if (uploaders.uploaderObjects[i].isUploading()) {
				return true;
			}
		}
	...
	fillHiddens				: function() {
		...
		for (var i = 0; i < uploaders.uploaderObjects.length; i++) {
			...
			if (uploaders.uploaderObjects[i].response != undefined) {

				if (uploaders.uploaderObjects[i].response.state == 'success') {

					var filename = uploaders.uploaderObjects[i].response.filename;
					var uuid = uploaders.uploaderObjects[i].response.uuid;

					$($(uploaders.uploaderObjects[i])[0]._element)
						.parent().parent().find('input:hidden').val(uuid + '|' + filename);

					$($(uploaders.uploaderObjects[i])[0]._element)
						.parent().parent().find('input:text').val(filename);

				} else {


					var myParent = $(uploaders.uploaderObjects[i].element).parent();
					myParent.addClass('error');
					myParent.append(
						$('<div/>').addClass('error-message').html(
							uploaders.uploaderObjects[i].response.message
						)
					);
...
qq.FileUploader = function(o){
    this._options = {
        ...
        template: '<div class="qq-uploader">' + 
               // '<div class="qq-upload-drop-area"><span>Drop files here to upload</span></div>' +
                '<div class="qq-upload-button"><img src="'+ base_url +'img/magnifying_glass.png" alt="Select a file" /></div>' +
                //'<ul class="qq-upload-list"></ul>' +
                //'<a href="#" id="ajaxUpload">Upload</a>' +
             '</div>',
	...
	//$(this._element).append(this._options.template);
	var targetInput = $(this._element).parent().find(".input");	
	this._element.innerHTML = this._options.template;	
    $(this._element).appendTo(targetInput);
...
qq.FileUploader.prototype = {
    uploadList: function() {
        if (this.files) {
            this._uploadFileList(this.files);
        }
    },
    ...
    _onInputChange: function(input){
        ...
        if (this._handler instanceof qq.UploadHandlerXhr){     
            this._writeFileList(input.files);
    ...
    _writeFileList: function(files) {
		if ($(".qq-upload-cancel",$(this._element)).length == 0) {
			$(this._element).parent().parent().find('input').css('width','221px');
			$(this._element).parent().css('margin-left','35');
			$(this._element).append('<a class="qq-upload-cancel" href="#"><img src="' + base_url + 'img/cross.png" alt="Cancel" /></a>');
			$(this._element).removeClass('error');
			$('.error-message', $(this._element)).remove();
			$(".qq-upload-cancel",$(this._element)).bind('click', function() {
				this.files = null;
				$(this).parent().parent().find('input[type="text"]').val('');
				$(this).parent().parent().find('input[type="text"]').css('width','247px');
				$(this).parent().parent().css('margin-left','60');
				$(this).remove();
			});
		}
		this.files = files;		
        ...
        while (i--) {
            text = text + '<li>' +
                '<span class="qq-upload-file">' + files[i].fileName  + '</span>' +
                '<span class="qq-upload-spinner"></span>' +
                '<span class="qq-upload-size"></span>' +
                '<a class="qq-upload-cancel" href="#">Cancel</a>' +
                '<span class="qq-upload-failed-text">Failed</span>' +
		...
		$("input[type='text']",this._element.parentNode).val(files[0].fileName);
		
    },
    _onButtonClick: function(input) {
    ...
    _bindCancelEvent: function(){
        /*var self = this,
			...
			$(self._classes.cancel,self._element).bind(function() {
				alert('algo');
			});
